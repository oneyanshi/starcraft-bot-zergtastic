...
/*		bwapi.drawCircle(mainHatchery.getPosition(), 5, BWColor.Yellow, true, false);
		...
		// Always: Make all drones collect minerals if we don't have an extractor
		...
		// Always: If we do have an extractor, then assign gasDrone1 and make it collect gas
		for (Unit unit : bwapi.getMyUnits()) {
		...
		if (step == 1) {
			for (Unit unit : bwapi.getMyUnits()) {
				if (unit.getType() == UnitTypes.Zerg_Larva) {
					if (bwapi.getSelf().getMinerals() >= 50 && buildCounter < 1) {
...
//			// Conditional step increase
//			ArrayList<Unit> anyEggs = new ArrayList<>();
//			for (Unit unit : bwapi.getMyUnits()) {
...
//			if (anyEggs.isEmpty()) {
//				step=4;
//				System.out.println("There are no more eggs");
//		}
			buildCounter = 0;
		}
...
//			}
		...
		if (step == 5) {
			for (Unit larva : bwapi.getMyUnits()) {
				...
				System.out.println("Larva : " + Integer.toString(Larva.size()));
				if (Larva.size() == 3) {
					for (Unit unit : bwapi.getMyUnits()) {
						if (unit.getType() == UnitTypes.Zerg_Larva) {
		...
		// 7. Spawn a second Overlord
		if (step == 7) {
			System.out.println("Step = " + Integer.toString(step));
			for (Unit larva : bwapi.getMyUnits()) {
				if (larva.getType() == UnitTypes.Zerg_Larva) {
					if (bwapi.getSelf().getMinerals() >= 100 && buildCounter < 1) {
		...
		if (step == 8) {
			for (Unit larva : bwapi.getMyUnits()) {
				...
				if (Larva.size() == 2) {
					for (Unit unit : bwapi.getMyUnits()) {
						if (unit.getType() == UnitTypes.Zerg_Larva) {
							if (bwapi.getSelf().getMinerals() >= 100 && buildCounter < 2) {
								unit.morph(UnitTypes.Zerg_Zergling);
								buildCounter++;
							}
						}
			...
			// 13. Spawn 2 Hydralisks
			if (step == 13) {
				for (Unit unit : bwapi.getMyUnits()) {
					if (unit.getType() == UnitTypes.Zerg_Hydralisk_Den && unit.isCompleted()) {
						for (Unit larva : bwapi.getMyUnits()) {
							if (larva.getType() == UnitTypes.Zerg_Larva) {
								if (bwapi.getSelf().getMinerals() >= 150 && bwapi.getSelf().getGas() >= 50 && buildCounter < 2) {
